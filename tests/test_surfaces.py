...
SURFACE_LIBS = {"argh", "cw", "click", "typer", "fastapi", "starlette", "uvicorn", "flask", "mcp", "fastmcp", "qh", "uf"}
...
def test_dispatch_funcs_are_json_shaped(tmp_path):
    ...
    assert names == ["calligram", "caption", "credits", "fonts", "lower_third", "overlay_video", "title_card"]
...
def test_tools_title_card_and_caption(tmp_path):
    ...
    r = title_card("Hello", "world", out=str(tmp_path / "t.png"), background="#000", size="640x360")
    assert (tmp_path / "t.png").exists() and r["bbox"][2] > r["bbox"][0]
    still = tmp_path / "still.jpg"
    Image.new("RGB", (640, 360), (40, 40, 40)).save(still)
    r = caption_tool(str(still), "A thing", "a source", out=str(tmp_path / "c.png"), avoid=None)
...
def test_tools_credits_cards_png(tmp_path):
    ...
    spec = json.dumps({"title": "T", "sections": [{"heading": "Cast", "entries": [["Role", "Name"]]}]})
...
def test_tools_calligram_png(tmp_path):
    ...
    r = calligram("round and round", out=str(tmp_path / "c.png"), shape="circle", size="400x400")
    assert r["glyphs"] == len("roundandround")


def test_cli_dispatch_lists_commands():
    pytest.importorskip("cw")
    out = subprocess.run([sys.executable, "-m", "tituli", "--help"], capture_output=True, text=True)
...
def test_video_still_overlay_crawl_frames(tmp_path):
    ...
    out = video.overlay(clip, [TimedOverlay(lay, 0.2, 0.8)], tmp_path / "over.mp4", size=size, workdir=tmp_path / "w")
    ...
    fr = frames(lay.staggered(step=0.1, ramp=0.1), Frame.blank(size, color="#000"), duration=0.5, fps=10)
...
def test_overlay_requires_a_layout(tmp_path):
    ...
    with pytest.raises(ValueError):
        video.overlay("x.mp4", [TimedOverlay(None, 0, 1)], tmp_path / "o.mp4", size=(10, 10))
...
def test_lacing_body_shape():
    ...
    assert set(b) >= {"text", "attribution", "kind", "anchor", "box", "ink", "unlabelled"}
